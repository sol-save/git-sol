  ...
  true &&
    it("create commit", async () => {
      ...
      try {
        const tx = await program.methods
          .addCommit({
            hash: "QmT5R7NVn6L6W7ArtkXBPEgCcR593jv5qhBN6MHaz7G8B4",
      ...
      console.log(
        "here ser",
        (await program.account.repoAccount.fetch(repoPDA)).commits
      );
      expect(
        Array((await program.account.repoAccount.fetch(repoPDA)).commits).length
      ).to.be.equal(1);
    });
});
